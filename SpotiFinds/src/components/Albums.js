        ...
        return(
            <main role="main" style={{minHeight: '90vh'}}>
                <section>
                    ...
                    <Row>
                        <Col xs={12}>
                            <section id="albumImage" style={{backgroundImage: `linear-gradient(0deg, rgba(1,1,1,1) 0%, rgba(255,255,255,0) 100%), url(${result.albums[0].images[0].url})`}}>
                                <h1 id='trackTitle'>{result.albums[0].name}</h1>
                                <p id='trackArtist'>{result.albums[0].artists[0].name}<Icon.Dot/>{result.albums[0].label}</p>
                            </section>
                        </Col>
                        ...
                        {tracks?.map(post =>
                            <section key={post.id} >
                                <Row id="records" className="rounded">
                                    <Col xs={4} style={{backgroundImage: `url(${result.albums[0].images[0].url})`}} id="recordImg"></Col>
                                    <Col as={Link} to={`/Tracks.js?spotifyID=${post.track.uri.slice(14)}`} className="d-flex text-decoration-none justify-content-center align-items-end">
                                        <Row>
                                            <Col xs={12}><h1 id="title">{post.track.name}</h1></Col>
                                            <Col xs={12}><p id="category">Song <Icon.Dot/>{post.track.artists.items[0].profile.name}</p></Col>
                                        </Row>
                                    </Col>
                                </Row>
                            </section>
